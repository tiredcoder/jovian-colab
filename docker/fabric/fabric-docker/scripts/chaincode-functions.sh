...
chaincodeBuild() {
  ...
  if [ "$CHAINCODE_LANG" = "node" ]; then
    ...
    if [ ! -n "$CHAINCODE_LANG_VERSION" ]; then
      echo "Notice: No Node.js version specified, defaulting to version $RECOMMENDED_NODE_VERSION."
    ...
    if ! echo "v$CHAINCODE_LANG_VERSION" | grep -q "v$RECOMMENDED_NODE_VERSION"; then
      echo "Warning: Your selected Node.js version is '$CHAINCODE_LANG_VERSION', but recommended is '$RECOMMENDED_NODE_VERSION')"
