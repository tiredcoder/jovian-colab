...
generateCryptoFabricCA() {
  ...
  # Fabric CA server port of each organization
  declare -A CA_SERVER_PORTS=( [orga.fabric.localhost]="7020" [orgb.fabric.localhost]="7040" [orgc.fabric.localhost]="7060" )
  ...
  inputLog "CA_SERVER_PORTS: $(echo ${CA_SERVER_PORTS[@]})"
  ...
  fabric-ca-client-enroll() {
    local ORG_DOMAIN="$1"
    local CA_CONTAINER="$2"
    local CA_SERVER_PORT="$3"
    local USER="$4"
    local USER_PW="$5"

    docker exec "$CA_CONTAINER" fabric-ca-client enroll -u "https://$USER:$USER_PW@localhost:$CA_SERVER_PORT" --caname "ca.$ORG_DOMAIN" --tls.certfiles "/etc/hyperledger/fabric-ca-server/ca-cert.pem"
  }
  ...
  fabric-ca-client-enroll-msp() {
    ...
    case "$ID_TYPE" in
      peer)
        docker exec "$CA_CONTAINER" fabric-ca-client enroll -u "https://$USER:$USER_PW@localhost:$CA_SERVER_PORT" --caname "ca.$ORG_DOMAIN" -M "/etc/hyperledger/fabric-ca-client/peers/$USER/msp" --csr.hosts "$USER" --tls.certfiles "/etc/hyperledger/fabric-ca-server/ca-cert.pem"
        ;;
      user)
        docker exec "$CA_CONTAINER" fabric-ca-client enroll -u "https://$USER:$USER_PW@localhost:$CA_SERVER_PORT" --caname "ca.$ORG_DOMAIN" -M "/etc/hyperledger/fabric-ca-client/users/$USER@$ORG/msp" --tls.certfiles "/etc/hyperledger/fabric-ca-server/ca-cert.pem"
        ;;
    esac
  }
  ...
  fabric-ca-client-enroll-tls() {
    ...
    docker exec "$CA_CONTAINER" fabric-ca-client enroll -u "https://$USER:$USER_PW@localhost:$CA_SERVER_PORT" --caname "ca.$ORG_DOMAIN" -M "/etc/hyperledger/fabric-ca-client/peers/$USER/tls" --enrollment.profile tls --csr.hosts "$USER" --csr.hosts localhost --tls.certfiles "/etc/hyperledger/fabric-ca-server/ca-cert.pem"
  ...
  for ORG in "${ORGs[@]}"; do
    ...
    fabric-ca-client-enroll "$ORG" "ca.$ORG" "${CA_SERVER_PORTS[$ORG]}" "${CA_ADMIN_NAMES[$ORG]}" "${CA_ADMIN_PASSWORDS[$ORG]}"
    createMspConfig "$ORG" "localhost-${CA_SERVER_PORTS[$ORG]}-ca-${ORG//./-}.pem"
  done
  ...
  for ORG in "${ORGs[@]}"; do
    ...
    for PEER in "peer0.$ORG" "peer1.$ORG"; do
      ...
      fabric-ca-client-enroll-msp "$ORG" "ca.$ORG" "${CA_SERVER_PORTS[$ORG]}" "peer" "$PEER" "peerpwsecret"
      ...
      fabric-ca-client-enroll-tls "$ORG" "ca.$ORG" "${CA_SERVER_PORTS[$ORG]}" "$PEER" "peerpwsecret"
  ...
  for ORG in "${ORGs[@]}"; do
    ...
    fabric-ca-client-enroll-msp "$ORG" "ca.$ORG" "${CA_SERVER_PORTS[$ORG]}" "peer" "$ORDERER" "ordererpwsecret"
    # Copy the org's MSP config file
    ...
    fabric-ca-client-enroll-tls "$ORG" "ca.$ORG" "${CA_SERVER_PORTS[$ORG]}" "$ORDERER" "ordererpwsecret"
  ...
  for ORG in "${ORGs[@]}"; do
    ...
    fabric-ca-client-enroll-msp "$ORG" "ca.$ORG" "${CA_SERVER_PORTS[$ORG]}" "user" "orgadmin" "orgadminpw"
    # Copy the org's MSP config file
...
installChannels() {
  ...
  docker exec -i cli.orga.fabric.localhost bash -c "source scripts/channel_fns.sh; createChannelAndJoinTls 'consortium-chain' 'MSPorgA' 'peer0.orga.fabric.localhost:7021' 'crypto/users/orgadmin@orga.fabric.localhost/msp' 'crypto/msp/tlscacerts/ca.crt' 'crypto-orderer/tlsca.orga.fabric.localhost-cert.pem' 'orderer0.consortium.orga.fabric.localhost:7030';"
  printItalics "Joining 'consortium-chain' on  orgA/peer1" "U1F638"
  docker exec -i cli.orga.fabric.localhost bash -c "source scripts/channel_fns.sh; fetchChannelAndJoinTls 'consortium-chain' 'MSPorgA' 'peer1.orga.fabric.localhost:7022' 'crypto/users/orgadmin@orga.fabric.localhost/msp' 'crypto/msp/tlscacerts/ca.crt' 'crypto-orderer/tlsca.orga.fabric.localhost-cert.pem' 'orderer0.consortium.orga.fabric.localhost:7030';"
  printItalics "Joining 'consortium-chain' on  orgB/peer0" "U1F638"
  docker exec -i cli.orgb.fabric.localhost bash -c "source scripts/channel_fns.sh; fetchChannelAndJoinTls 'consortium-chain' 'MSPorgB' 'peer0.orgb.fabric.localhost:7041' 'crypto/users/orgadmin@orgb.fabric.localhost/msp' 'crypto/msp/tlscacerts/ca.crt' 'crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem' 'orderer0.consortium.orgb.fabric.localhost:7050';"
  printItalics "Joining 'consortium-chain' on  orgB/peer1" "U1F638"
  docker exec -i cli.orgb.fabric.localhost bash -c "source scripts/channel_fns.sh; fetchChannelAndJoinTls 'consortium-chain' 'MSPorgB' 'peer1.orgb.fabric.localhost:7042' 'crypto/users/orgadmin@orgb.fabric.localhost/msp' 'crypto/msp/tlscacerts/ca.crt' 'crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem' 'orderer0.consortium.orgb.fabric.localhost:7050';"
  printItalics "Joining 'consortium-chain' on  orgC/peer0" "U1F638"
  docker exec -i cli.orgc.fabric.localhost bash -c "source scripts/channel_fns.sh; fetchChannelAndJoinTls 'consortium-chain' 'MSPorgC' 'peer0.orgc.fabric.localhost:7061' 'crypto/users/orgadmin@orgc.fabric.localhost/msp' 'crypto/msp/tlscacerts/ca.crt' 'crypto-orderer/tlsca.orgc.fabric.localhost-cert.pem' 'orderer0.consortium.orgc.fabric.localhost:7070';"
  printItalics "Joining 'consortium-chain' on  orgC/peer1" "U1F638"
  docker exec -i cli.orgc.fabric.localhost bash -c "source scripts/channel_fns.sh; fetchChannelAndJoinTls 'consortium-chain' 'MSPorgC' 'peer1.orgc.fabric.localhost:7062' 'crypto/users/orgadmin@orgc.fabric.localhost/msp' 'crypto/msp/tlscacerts/ca.crt' 'crypto-orderer/tlsca.orgc.fabric.localhost-cert.pem' 'orderer0.consortium.orgc.fabric.localhost:7070';"
...
installChaincodes() {
  if [ -n "$(ls "$CHAINCODES_DIR/cc-ipfs")" ]; then
    ...
    chaincodePackage "cli.orga.fabric.localhost" "peer0.orga.fabric.localhost:7021" "consortium-cc-ipfs" "$version" "node" printHeadline "Installing 'consortium-cc-ipfs' for orgA" "U1F60E"
    chaincodeInstall "cli.orga.fabric.localhost" "peer0.orga.fabric.localhost:7021" "consortium-cc-ipfs" "$version" "crypto-orderer/tlsca.orga.fabric.localhost-cert.pem"
    chaincodeInstall "cli.orga.fabric.localhost" "peer1.orga.fabric.localhost:7022" "consortium-cc-ipfs" "$version" "crypto-orderer/tlsca.orga.fabric.localhost-cert.pem"
    chaincodeApprove "cli.orga.fabric.localhost" "peer0.orga.fabric.localhost:7021" "consortium-chain" "consortium-cc-ipfs" "$version" "orderer0.consortium.orga.fabric.localhost:7030" "" "false" "crypto-orderer/tlsca.orga.fabric.localhost-cert.pem" ""
    printHeadline "Installing 'consortium-cc-ipfs' for orgB" "U1F60E"
    chaincodeInstall "cli.orgb.fabric.localhost" "peer0.orgb.fabric.localhost:7041" "consortium-cc-ipfs" "$version" "crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem"
    chaincodeInstall "cli.orgb.fabric.localhost" "peer1.orgb.fabric.localhost:7042" "consortium-cc-ipfs" "$version" "crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem"
    chaincodeApprove "cli.orgb.fabric.localhost" "peer0.orgb.fabric.localhost:7041" "consortium-chain" "consortium-cc-ipfs" "$version" "orderer0.consortium.orgb.fabric.localhost:7050" "" "false" "crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem" ""
    printHeadline "Installing 'consortium-cc-ipfs' for orgC" "U1F60E"
    chaincodeInstall "cli.orgc.fabric.localhost" "peer0.orgc.fabric.localhost:7061" "consortium-cc-ipfs" "$version" "crypto-orderer/tlsca.orgc.fabric.localhost-cert.pem"
    chaincodeInstall "cli.orgc.fabric.localhost" "peer1.orgc.fabric.localhost:7062" "consortium-cc-ipfs" "$version" "crypto-orderer/tlsca.orgc.fabric.localhost-cert.pem"
    chaincodeApprove "cli.orgc.fabric.localhost" "peer0.orgc.fabric.localhost:7061" "consortium-chain" "consortium-cc-ipfs" "$version" "orderer0.consortium.orgc.fabric.localhost:7070" "" "false" "crypto-orderer/tlsca.orgc.fabric.localhost-cert.pem" ""
    printItalics "Committing chaincode 'consortium-cc-ipfs' on channel 'consortium-chain' as 'orgA'" "U1F618"
    chaincodeCommit "cli.orga.fabric.localhost" "peer0.orga.fabric.localhost:7021" "consortium-chain" "consortium-cc-ipfs" "$version" "orderer0.consortium.orga.fabric.localhost:7030" "" "false" "crypto-orderer/tlsca.orga.fabric.localhost-cert.pem" "peer0.orga.fabric.localhost:7021,peer0.orgb.fabric.localhost:7041,peer0.orgc.fabric.localhost:7061" "crypto-peer/peer0.orga.fabric.localhost/tls/ca.crt,crypto-peer/peer0.orgb.fabric.localhost/tls/ca.crt,crypto-peer/peer0.orgc.fabric.localhost/tls/ca.crt" ""
  else
  ...
  if [ -n "$(ls "$CHAINCODES_DIR/cc-ipfs")" ]; then
    ...
    chaincodePackage "cli.orgb.fabric.localhost" "peer0.orgb.fabric.localhost:7041" "orgb-cc-ipfs" "$version" "node" printHeadline "Installing 'orgb-cc-ipfs' for orgB" "U1F60E"
    chaincodeInstall "cli.orgb.fabric.localhost" "peer0.orgb.fabric.localhost:7041" "orgb-cc-ipfs" "$version" "crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem"
    chaincodeInstall "cli.orgb.fabric.localhost" "peer1.orgb.fabric.localhost:7042" "orgb-cc-ipfs" "$version" "crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem"
    chaincodeApprove "cli.orgb.fabric.localhost" "peer0.orgb.fabric.localhost:7041" "orgb-chain" "orgb-cc-ipfs" "$version" "orderer0.orgb.orgb.fabric.localhost:7051" "AND ('MSPorgB.member')" "false" "crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem" ""
    printItalics "Committing chaincode 'orgb-cc-ipfs' on channel 'orgb-chain' as 'orgB'" "U1F618"
    chaincodeCommit "cli.orgb.fabric.localhost" "peer0.orgb.fabric.localhost:7041" "orgb-chain" "orgb-cc-ipfs" "$version" "orderer0.orgb.orgb.fabric.localhost:7051" "AND ('MSPorgB.member')" "false" "crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem" "peer0.orgb.fabric.localhost:7041" "crypto-peer/peer0.orgb.fabric.localhost/tls/ca.crt" ""
  else
...
notifyOrgsAboutChannels() {
  ...
  notifyOrgAboutNewChannelTls "consortium-chain" "MSPorgA" "cli.orga.fabric.localhost" "peer0.orga.fabric.localhost" "orderer0.consortium.orga.fabric.localhost:7030" "crypto-orderer/tlsca.orga.fabric.localhost-cert.pem"
  ...
  notifyOrgAboutNewChannelTls "consortium-chain" "MSPorgC" "cli.orgc.fabric.localhost" "peer0.orgc.fabric.localhost" "orderer0.consortium.orgc.fabric.localhost:7070" "crypto-orderer/tlsca.orgc.fabric.localhost-cert.pem"
  notifyOrgAboutNewChannelTls "orgb-chain" "MSPorgB" "cli.orgb.fabric.localhost" "peer0.orgb.fabric.localhost" "orderer0.orgb.orgb.fabric.localhost:7051" "crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem"
...
upgradeChaincode() {
  ...
  if [ "$chaincodeName" = "orgb-cc-ipfs" ]; then
    if [ -n "$(ls "$CHAINCODES_DIR/cc-ipfs")" ]; then
      ...
      chaincodeInstall "cli.orgb.fabric.localhost" "peer0.orgb.fabric.localhost:7041" "orgb-cc-ipfs" "$version" "crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem"
      chaincodeInstall "cli.orgb.fabric.localhost" "peer1.orgb.fabric.localhost:7042" "orgb-cc-ipfs" "$version" "crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem"
      chaincodeApprove "cli.orgb.fabric.localhost" "peer0.orgb.fabric.localhost:7041" "orgb-chain" "orgb-cc-ipfs" "$version" "orderer0.orgb.orgb.fabric.localhost:7051" "" "false" "crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem" ""
      printItalics "Committing chaincode 'orgb-cc-ipfs' on channel 'orgb-chain' as 'orgB'" "U1F618"
      chaincodeCommit "cli.orgb.fabric.localhost" "peer0.orgb.fabric.localhost:7041" "orgb-chain" "orgb-cc-ipfs" "$version" "orderer0.orgb.orgb.fabric.localhost:7051" "" "false" "crypto-orderer/tlsca.orgb.fabric.localhost-cert.pem" "peer0.orgb.fabric.localhost:7041" "crypto-peer/peer0.orgb.fabric.localhost/tls/ca.crt" ""
